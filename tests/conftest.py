...
from jenna_stream_orders import JennaStreamOrders, DefaultAioHttpClient, AsyncJennaStreamOrders
from jenna_stream_orders._utils import is_dict
...
logging.getLogger("jenna_stream_orders").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[JennaStreamOrders]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with JennaStreamOrders(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncJennaStreamOrders]:
